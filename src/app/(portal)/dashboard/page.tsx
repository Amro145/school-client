        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <div className="flex items-center space-x-4 mb-2">
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none">
                            Hello, <span className="text-purple-600">{user.userName.split(' ')[0]}</span>
                        </h1>
                        {isTopPerformer && (
                            <span className="bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-amber-200 flex items-center">
                                <Star className="w-3 h-3 mr-1 fill-amber-700" /> Top Performer
                            </span>
                        )}
                    </div>
                    <p className="text-slate-500 font-medium text-lg italic tracking-tight">Your academic progress is being tracked.</p>
                </div>
                <div className="flex items-center space-x-3 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <div className="pr-4">
                        ...
                        <p className="text-sm font-bold text-slate-900">Enrolled Student</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="glass p-8 rounded-[40px] border-slate-100 flex flex-col justify-between group hover:border-blue-200 transition-all duration-500 shadow-xl shadow-slate-100/50">
                    ...
                    <div>
                        <div className="text-2xl font-black text-slate-900 tracking-tighter mb-1">{user.class?.name || 'No Class Assigned'}</div>
                        <p className="text-slate-500 font-bold text-sm">{user.class?.subjects?.length || 0} Subjects Enrolled</p>
                    </div>
                </div>

                {/* Success Rate Card */}
                <div className="glass p-8 rounded-[40px] border-slate-100 flex flex-col justify-between group hover:border-green-200 transition-all duration-500 shadow-xl shadow-slate-100/50">
                    <div className="flex items-center justify-between mb-8">
                    ...
                    <div>
                        <div className="text-5xl font-black text-slate-900 tracking-tighter mb-1 tabular-nums">{averageScore ? Math.floor(averageScore * 10) / 10 : 'N/A'}%</div>
                ...
                <div className="bg-slate-900 p-8 rounded-[40px] border border-white/5 flex flex-col justify-between group overflow-hidden relative shadow-2xl">
                    ...
                    <div className="relative z-10">
                        <div className="text-5xl font-black text-white tracking-tighter mb-1 tabular-nums">{averageScore}</div>
            ...
            <div className="glass p-8 rounded-[48px] border-slate-100 shadow-xl shadow-slate-100/50">
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center">
                        <Award className="w-6 h-6 mr-3 text-amber-500" />
                        Academic Modules
                    </h2>
                    <Link href="/profile" className="text-xs font-black text-purple-600 uppercase tracking-widest hover:underline flex items-center group">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {user.class?.subjects?.slice(0, 3).map((subject) => {
                        ...
                        return (
                            <div key={subject.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:border-purple-200 hover:shadow-xl transition-all group/card">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    {grade && (
                                        <span className={`text-xs font-black px-2 py-1 rounded-lg ${grade.score >= 50 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                                            {grade.score}
                                        </span>
                                    )}
                                </div>
